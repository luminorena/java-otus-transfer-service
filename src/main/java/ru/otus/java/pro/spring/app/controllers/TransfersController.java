...
import ru.otus.java.pro.spring.app.dtos.ExecuteTransfersDtoRs;
...
public class TransfersController {
    private static final Function<Transfer, TransferDto> ENTITY_TO_TRANSFER_DTO = t -> new TransferDto(t.getId(), t.getClientId(), t.getTargetClientId(), t.getSourceAccount(), t.getTargetAccount(), t.getMessage(), t.getAmount());
    ...
    public TransfersPageDto getAllTransfers(@RequestHeader(name = "client-id") String clientId) {
        return new TransfersPageDto(
                transfersService
                        ...
                        .stream()
                        .map(ENTITY_TO_TRANSFER_DTO).collect(Collectors.toList())
        );
    }

    @GetMapping("/{id}")
    public TransferDto getTransferById(@RequestHeader(name = "client-id") String clientId, @PathVariable String id) {
        return ENTITY_TO_TRANSFER_DTO.apply(transfersService.getTransferById(id, clientId).orElseThrow(() -> new ResourceNotFoundException("Перевод не найден")));
    }
    ...
    public ResponseEntity<Transfer> executeTransfer(
        ...
        Transfer transfer = transfersService.execute(clientId, executeTransferDtoRq);
